...
def build_topology(net):
    ...
    net.addLink(mb2, s2, cls=TCLink, bw=100, delay="1ms")  # SOLUTION: connect mb2
...
def main():
    ...
    try:
        ...
        # SOLUTION: include mb2 in SRv6 configuration
        ...
        time.sleep(0.5)
        # SOLUTION: chain=["mb1", "mb2"] produces segments fc00::b1,fc00::b2,fc00::2
        # mb1 must come before mb2 because the SRH is processed left-to-right.
        # If mb2 didn't have a neighbour entry for fc00::2, it would drop the
        # packet after processing its segment — silent mid-chain failure.
        sc.provision("premium", src="h1", dst="h2", chain=["mb1", "mb2"], bw=8)
        sc.status()
