...
Then verify:
    sudo python3 exercises/part2/verify.py
"""
...
def main():
    ...
    ok &= check(
        ...
        "actions=" in flows,
        "No rules found — fill in and run: sudo bash exercises/part2/add_rules.sh",
    )
    ...
    out = run_in_host("h1", ["ping", "-c", "2", "-W", "1", "10.0.0.3"])
    ok &= check("h1 can reach h3", packet_loss(out) == 0, out)

    out = run_in_host("h3", ["ping", "-c", "2", "-W", "1", "10.0.0.1"])
    ok &= check("h3 can reach h1", packet_loss(out) == 0, out)

    out = run_in_host("h2", ["ping", "-c", "2", "-W", "1", "10.0.0.3"])
    ok &= check("h2 cannot reach h3", packet_loss(out) == 100, out)

    out = run_in_host("h3", ["ping", "-c", "2", "-W", "1", "10.0.0.2"])
    ok &= check("h3 cannot reach h2", packet_loss(out) == 100, out)
